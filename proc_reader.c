...
int show_system_info(void) {
    ...
    while (fgets(buffer, sizeof(buffer), cpu) && line_count < MAX_LINES) {
        printf("%s", buffer);
        line_count++;
    }
    ...
    line_count = 0;
    while (fgets(buffer, sizeof(buffer), mem) && line_count < MAX_LINES) {
        printf("%s", buffer);
        line_count++;
    }
...
int read_file_with_syscalls(const char* filename) {
    ...
    while ((bytes_read = read(fd, buffer, sizeof(buffer) - 1)) > 0) {
        ...
        /* Replace nulls (important for /proc/[pid]/cmdline) */
